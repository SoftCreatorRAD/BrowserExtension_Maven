function getParsed(target, cardSel, priceSel) {
  let dataObj = null;

  if (target && target.classList.contains('rapidapprover-btn')) {
    const domain = window.location.hostname.split('.').reverse()[1];
    let big = target.classList.contains('rapidapprover-' + domain + '-big') ? 'Big' : '';
    let card = target.closest(cardSel[domain + big]);
    ...
    // Monthly Payment parser (big card only):
